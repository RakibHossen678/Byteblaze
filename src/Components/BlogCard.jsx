import { Link } from "react-router-dom";
import placeHolderImage from "../assets/404.jpg";
import PropTypes from "prop-types";
import { MdDeleteForever } from "react-icons/md";


const BlogCard = ({ blog ,deleteTable,handleDeleteBlog}) => {
  
  const { description, published_at, cover_image, title, id } = blog;
  return (
    <div className="flex relative">
      <Link
        to={`/blog/${id}`}
        className="max-w-sm mx-auto group hover:no-underline focus:no-underline transition border-2 hover:scale-105 border-primary hover:border-secondary border-opacity-30 rounded-lg "
      >
        <img
          role="presentation"
          className="object-cover w-full rounded h-44 bg-gray-500"
          src={cover_image || placeHolderImage}
        />
        <div className="p-6 space-y-2">
          <h3 className="text-2xl font-semibold group-hover:underline group-focus:underline">
            {title}
          </h3>
          <span className="text-xs text-gray-400">
            {new Date(published_at).toLocaleDateString()}
          </span>
          <p>{description}</p>
        </div>
      </Link>
      {
        deleteTable &&( <div
        onClick={()=>handleDeleteBlog(id)}
         className="absolute -top-5 -right-5 bg-primary hover:scale-105 p-3 rounded-full hover:bg-opacity-40"><MdDeleteForever size={20}  className="text-secondary  hover:text-primary "/></div>)
      }
      <div>

      </div>
    </div>
  );
};
BlogCard.propTypes = {
  blog: PropTypes.object,
  deleteTable:PropTypes.bool,
  handleDeleteBlog:PropTypes.func
};

export default BlogCard;
